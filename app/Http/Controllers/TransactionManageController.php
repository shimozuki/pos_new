    ...
    public function transactionProcess(Request $req)
    {
        ...
        if ($check_access->transaksi == 1) {
            if ($req->bayar < $req->total) {
                ...
                Session::flash('transaction_warning', $req->kode_transaksi);
                return back();
            } else {
                $jml_barang = count($req->kode_barang);
                for ($i = 0; $i < $jml_barang; $i++) {
    ...
    public function receiptTransaction($id)
    {
        ...
        if ($check_access->transaksi == 1) {
            ...
            if ($transaction->bayar < $transaction->total) {
                $pdf = PDF::loadview('transaction.receipt_utang', compact('transaction', 'transactions', 'diskon', 'market'))->setPaper($customPaper, 'landscape');
                return $pdf->stream();
            } else {
